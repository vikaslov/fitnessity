...
{   
    ...
    public function index()
    {
        ...
        return view('admin.professionals.index', [
            'pageTitle'     => 'Manage Professionals',
    ...
    public function view($id)
    {
        ...
        return view('admin.professionals.view', [
            'pageTitle'     => 'View Professional Details',
    ...
    public function postProfessionals(Request $request){
        ...
        if(count(@$input['professionalIds']) > 0 && @$input['submit_delete_professional'] == 1){
            ...
            if(!$update) {
                $response = array(
                        'danger' => 'Some error while deleting Professionals.',
                );
            } else {
                $response = array(
                    'success' =>  'Professionals successfully deleted.',
                );    
            }

        } else {
            $response = array(
                    'danger' =>  'Please select at least one Professional.',
            );
        }
        ...
        if(count(@$input['professionalIds']) > 0 && @$input['submit_approve_professional'] == 1){
            ...
            if(!$update) {
                $response = array(
                        'danger' => 'Some error while approving Professionals.',
                );
            } else {
                $response = array(
                    'success' =>  'Approved Professionals Successfully!',
                );    
            }
