...
@section('content')
    ...
    <p>
        ...
        <a href="{!! route('create-new-discover') !!}" class="btn btn-success">Add New</a>
    ...
    <div class="panel panel-default">
        ...
        {!! Form::open(array('route' => 'delete-discovers', 'id' => 'discoverlist')) !!}

            <div class="table-responsive">
              <table id="slider_list" class="table table-bordered table-striped {{ count($discovers) > 0 ? 'datatable' : '' }} table-hover ">
                ...
                <tbody>
                    ...
                    <?php $i = 0 ?>
                        ...
                        @foreach ($discovers as $value)

                          <?php $i++ ?>

                            <tr id="item-{!! $value->id !!}">

                                <td><input type="checkbox" name="discoverIds[]" value="{{$value->id}}"></td>

                                <td><img src="{{ asset('public/uploads/discover/thumb/'.$value->image) }}" width="150" height="auto" /></td>

                                <td><a href="\admin\discover\edit\{{$value->id}}" title="Click to edit {{@$value->title}}">{{@$value->title}}</a></td>

                                <td>{{@$value->description}}</td>

                  <td>

                      <a href="\admin\discover\edit\{{$value->id}}" title="Click to edit {{@$value->title}}" class="btn btn-xs btn-primary"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                 ...
                 <tfoot>
                    ...
                    <button type="submit" id="submit_delete_discover" name="submit_delete_discover" class="btn btn-danger btn-xs" title="Deactivate Selected Plans" onclick="return confirm('Do you really want to deactivate selected Plans?');" value="1"><i class="fa fa-ban" aria-hidden="true"></i></button>
