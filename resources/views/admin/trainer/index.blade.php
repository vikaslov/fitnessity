...
@section('content')
    ...
    <p>
        ...
        <a href="{!! route('create-new-trainer') !!}" class="btn btn-success">Add New</a>
    ...
    <div class="panel panel-default">
        ...
        {!! Form::open(array('route' => 'delete-trainers', 'id' => 'trainerlist')) !!}

            <div class="table-responsive">
              <table id="slider_list" class="table table-bordered table-striped {{ count($trainers) > 0 ? 'datatable' : '' }} table-hover ">
                        ...
                        @foreach ($trainers as $value)
                          ...
                          <?php $i++ ?>
                            ...
                            <tr id="item-{!! $value->id !!}">
                                ...
                                <td><input type="checkbox" name="trainerIds[]" value="{{$value->id}}"></td>

                                <td><img src="{{ asset('public/uploads/trainer/thumb/'.$value->image) }}" width="150" height="auto" /></td>

                                <td><a href="\admin\plans\edit\{{$value->id}}" title="Click to edit {{@$value->name}}">{{@$value->name}}</a></td>

                  <td>

                      <a href="\admin\trainer\edit\{{$value->id}}" title="Click to edit {{@$value->name}}" class="btn btn-xs btn-primary"><i class="fa fa-pencil" aria-hidden="true"></i></a>

                      <a href="\admin\trainer\delete\{{$value->id}}" title="Click to delete {{@$value->name}}" onclick="return confirm('Are you sure?')" class="btn btn-xs btn-primary"><i class="fa fa-trash" aria-hidden="true"></i></a>
